...
const getFloorY = (floorLabel) => {
  ...
  if (index === -1) return 0
  // 使用 justify-content: space-between 的计算方式
  // (index / (length - 1)) * (H - rowHeight) + rowHeight / 2
...
const nodeX_Left = 14  // node at 8 + half of 12 is 14
const nodeX_Right = 164 // node at 158 + half of 12 is 164
...
const getPathData = (task) => {
  ...
  if (task.side === 'left') {
    // 左侧电梯：从轨道右边缘 (28) 出发 -> 偏移量 -> 目标层 -> 指向节点 (14)
    const startX = 28
    const horizontalOffset = task.type === 'solid' ? 70 : 90
    const targetX = nodeX_Left
    return `M ${startX} ${startY} L ${horizontalOffset} ${startY} L ${horizontalOffset} ${endY} L ${targetX} ${endY}`
  } else {
    // 右侧电梯：从轨道左边缘 (150) 出发 -> 偏移量 -> 目标层 -> 指向节点 (164)
    const startX = 150
...
<template>
  <div class="side-panel left-panel">
    <div class="panel-title">
      <span class="title-text">非洁净电梯任务展示</span>
...
.floor-list {
  ...
  .floor-item {
      ...
      background: linear-gradient(90deg, rgba(0, 247, 255, 0.15) 0%, rgba(0, 247, 255, 0.05) 100%);
    border: 1px solid;
    border-image: linear-gradient(90deg, rgba(0, 247, 255, 0.5), rgba(0, 247, 255, 0.1)) 1;
    display: flex;
    flex-direction: column;
    ...
    &.active {
      background: linear-gradient(90deg, rgba(0, 255, 0, 0.3) 0%, rgba(0, 255, 0, 0.1) 100%);
      border-image: linear-gradient(90deg, rgba(0, 255, 0, 0.8), rgba(0, 255, 0, 0.2)) 1;
    ...
    .track-floors {
      ...
      .track-row {
        ...
        .node {
          ...
          &.left-node { left: 8px; top: 50%; transform: translateY(-50%); }
          &.right-node { left: 158px; top: 50%; transform: translateY(-50%); }
